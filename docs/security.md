...
The matchmaker checks the version code against a hash. If the hashes match, it checks how many games are in progress (and potentially in the future also how many players have been assigned to each game and current CPU usage). If all are within the set limits, the matchmaker creates a new game.[^4] It picks a port number from a pool of ten (7777–7786), then generates a passcode, consisting of six random digits, and a longer private key, consisting of 32 random bytes.
