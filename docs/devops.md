...
- **`BUTLER_API_KEY`**
  - Used in `deploy-client.yaml` as `BUTLER_API_KEY`, the authentication token Butler uses to push builds to itch.io.
...
- **`deploy_frontend.sh`** triggers the `Deploy Client` workflow (`repository_dispatch` with `deploy_client`), which pushes client builds to itch.io and to GitHub Releases. It runs from cron as `non-root-user` at 04:01.
- **`deploy_backend.sh`** fetches `docker-compose.yaml` and `Caddyfile` from GitHub (`main`) into `/home/non-root-user`, pulls the server image from Docker Hub, and runs `docker compose up -d` there. It runs from cron as `non-root-user` at 04:30, then reboots if `/var/run/reboot-required` exists.
...
```bash
cd ~/scripts
./deploy_frontend.sh   # trigger client → itch.io + GitHub Releases
...
| 04:01 | Webhook fires to deploy client artifacts to itch.io and GitHub Releases | deploy_frontend.sh (Cron, root user) |
