...
mod tests {
    ...
    #[test]
    fn attempts_do_not_overflow_when_already_at_max() {
        let passcode = [1, 2, 3, 4, 5, 6];
        let mut attempts = u8::MAX;
        let outcome = evaluate_passcode_attempt(&passcode, &mut attempts, &[0, 0, 0, 0, 0, 0], 3);
        assert_eq!(outcome, AuthAttemptOutcome::Disconnect);
    ...
    #[test]
    fn server_state_tracks_authentication_attempts_per_client() {
...

        {
            let attempts = state
                ...
                .expect("registered client should have entry");
            *attempts = 2;
        }

        if let Some(attempts) = state.authentication_attempts(42) {
            assert_eq!(*attempts, 2);
        } else {
            panic!("registered client should still be tracked");
        }

        state.remove_client(42);
        assert!(!state.is_authenticating(42));
