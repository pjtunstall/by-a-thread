#!/usr/bin/env bash
# Build the client for Apple Silicon (aarch64) and create a .app bundle.
...
APP_NAME=ByAThread
BUNDLE=$APP_NAME.app
STAGING=dist/$APP_NAME-macos-silicon

echo "Building for Apple Silicon ($TARGET)..."
...
(cd dist && zip -r "${APP_NAME}-macos-silicon.zip" "$(basename "$STAGING")")
rm -rf "$STAGING"
echo "Created $BUNDLE and dist/${APP_NAME}-macos-silicon.zip"
