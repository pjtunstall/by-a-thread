...
In production, an ephemeral private key for each game will be generated by a matchmaker (in this case, just a program for launching games rather than a matchmaker in the strict sense). A would-be host will request a game from the matchmaker. If a slot is available (i.e. less than the maximum number of game servers are running), the matchmaker will create a random private key and launch an instance of the game server with that key as an environment variable. The matchmaker will generate a token from the key and pass the token, along with the game's port number and an ephemeral passcode, unique to the game, to the host.

The host will then connect to the game server using the token and port number. As they're the first player to connect, the server will mark them as the host. Meanwhile, the host can share the passcode with their friends.

These other players will send the passcode to the matchmaker. If it's valid, the matchmaker will reply with the token and port. They'll use these to connect to the server, which will admit them provided the token is valid and there's less than the maximum number of players, and the game has not begun yet.

When the host has chosen a difficulty level, the server will report to the matchmaker so that it can update the game's status to "in progress". Once the matchmaker has sent an acknowledgement back to the server, the latter will proceed with the game proper.
