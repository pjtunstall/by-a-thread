...
In production, an ephemeral private key for each game will be generated by a matchmaker (in this case, just a program for launching games rather than a matchmaker in the strict sense).

In detail: A would-be host will request a game from the matchmaker. If a slot is available (i.e. less than the maximum number of game servers are running), the matchmaker will create a random private key and a reporting token, and launch an instance of the game server, passing these secrets to it as an environment variables.

Meanwhile, the matchmaker will generate a connect token from the key and pass this, along with the game's port number and an ephemeral passcode, unique to the game, to the host.

As soon as the host receives this data, they will automatically connect to the game server using the connect token and port number. As this client is the first player to connect, the server will mark them as the host. The host can then share the passcode with their friends.
...
When the host has chosen a difficulty level, the server will report to the matchmaker, identifying itself with the reporting token. This will allow the matchmaker to update the game's status to "in progress". Once the matchmaker has sent an acknowledgement back to the server, the latter can proceed with the game proper.
